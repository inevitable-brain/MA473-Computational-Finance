...
basis = @(x_min, x_max, N, i, x) linear_basis(x_min, x_max, N, i, x);

der_b = @(x_min, x_max, N, i, x, flag) derivative_linear_basis(x_min, x_max, N, i, x, flag);
          
for q_idx = 1 : length(quadrature_rules)
    fprintf("\nSolving BVP using %s\n\n", disp_helper(q_idx));
    
    quadrature = deal(quadrature_rules{q_idx});
    ...
    xlabel("x");
    ylabel("u");
    title(disp_helper(q_idx));
    saveas(gcf, strcat(disp_helper(q_idx), '2.png'));
end
